...
Based on the line graph of the original data below (Figure 1), we can observe that exponentially growing variables need to be linearized, such as cpi, gdp. Therefore, its logarithmically transformed form will be used in the following analysis.
...
```{r Log transfomation for gdp and cpi}
...
According to Table 1, we can see that $cpi_i$, $gdp_i$, $gold_i$, $aord_i$, and $exr_i$ require second-order integration, while other variables require first-order integration.
...
# ol.cpi.aic.ar$order
adf.cpi   = adfTest(lcpi_data, lags=19, type="c")              # don't reject -> non-stationary
dadf.cpi  = adfTest(diff(lcpi_data), lags=18, type="nc")        # don't reject -> non-stationary
d2adf.cpi = adfTest(diff(diff(lcpi_data)), lags=17, type="nc")  # reject -> (I2 is stationary)
...
# ol.gdp.aic.ar$order
adf.gdp   = adfTest(lgdp_data, lags=19, type="c")              # don't reject -> non-stationary
dadf.gdp  = adfTest(diff(lgdp_data), lags=18, type="nc")       # don't reject -> non-stationary
d2adf.gdp = adfTest(diff(diff(lgdp_data)), lags=17, type="nc")  # reject -> (I2 is stationary)
...
Since the second-order difference may remove part of the long-term information (such as trends) in the data, this may be detrimental to time series data such as $cpi_i$ and $gdp_i$ that contain important trend information. Therefore, only first-order differences are used for all data.
