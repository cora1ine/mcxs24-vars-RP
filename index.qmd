...
posterior.draws.exten.hetero = function (total_S, Y, X){
  ...
  for (s in 1:total_S){
      ...
      aux$Sigma              = Sigma.posterior.draw
      A.norm = matrix(rnorm(K * N), K, N)
      L                      = t(chol(V.bar))
      aux$A                  = A.bar + L%*%A.norm%*%chol(aux$Sigma)
      ...
      if (s < total_S) {
        ...
        product = t(diff_A) %*% solve(V.prior) %*% diff_A
        chi.b = sum(diag(solve(aux$Sigma %*% product)))
        psi.a = 2 / lambda.priors$theta
        aux$lambda[s+1] = GIGrvg::rgig(n = 1, lambda = lambda.p, chi = chi.b, psi = psi.a)
    }
      ...
      posteriors$A[,,s]            = aux$A
      posteriors$Sigma[,,s]        = aux$Sigma
      posteriors$lambda[s]        = aux$lambda[s]
...
A.posterior.ext.simu       = posterior.exten.hetero[["A"]]
Sigma.posterior.ext.simu   = posterior.exten.hetero[["Sigma"]]
```
...
#| message: false
#| warning: false
#| fig-cap: "Figure 16 3D forecasting graph on extended model with heteroskedasticity"
