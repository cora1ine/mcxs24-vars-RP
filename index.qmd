...
legend("topright", legend = c("Inflation Rate", "Expected Inflation Rate"),
       col = c(blue3, blue4), lty = 1, cex = 0.6)
...
point.lcpi.h            = ts(round(apply(Y.h[,1,],1,mean),6))
...
lines(data_df$Index[1:num], data_df$infexp_data.h[1:num], col = blue4)

lines(data_df$Index[num:length(data_df$Index)], data_df$inf_data.h[num:length(data_df$Index)], col = green2)
lines(data_df$Index[num:length(data_df$Index)], data_df$infexp_data.h[num:length(data_df$Index)], col = green3)

abline(v = data_df$Index[136], col = green1, lty = 1) 
...
legend("topright", legend = c("Inflation Rate", "Expected Inflation Rate", "Inflation Rate Forecast", "Expected Inflation Rate Forecast", "2023-12-31"),
       col = c(blue1, blue4, green2, green3, green1), lty = 1, cex = 0.6)
...
```{r Extension Function Foracasting}
## two-year ahead forecasting h=8
# set up
h                 = 8
S                 = 9000
...
## transform cpi into inflation rate
point.lcpi.h.ext            = ts(round(apply(Y.h.ext[,1,],1,mean),6))
point.lcpi.h.ext            = xts(point.lcpi.h, order.by = dates)
