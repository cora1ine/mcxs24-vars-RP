...
Given that differencing might eliminate some of the long-term information (like trends) within the data, and considering the Minnesota prior method can be effective with nonstationary data, we have opted apply not differencing to any of the data sets.
...
par(mfcol = c(1, 1), mar = c(4, 4, 2, 2) + 0.5)
...
#| warning: false
## Function Applying
posterior.sample.draws = posterior.draws(S=50000, Y=Y, X=X) # sample use S=50000

# Presenting output

# head(round(apply(posterior.sample.draws$A.posterior, 1:2, mean),5))           # posterior draw A
# head(round(apply(posterior.sample.draws$Sigma.posterior, 1:2, mean),5))       # posterior draw sigma
```
...
hist(posterior.sample.draws[["A.posterior"]][2,6,], xlab = "M3", col = blue2, main = '')
...
hist(posterior.sample.draws[["Sigma.posterior"]][1,6,], xlab = "M3 sigma", col = green2, main = '')
```

Figure 6 Model Simulation of Basic Model
...
hist(posterior.extend.draws[["A.posterior.exten"]][2,6,][(S1+1):total_S], xlab = "M3", col = blue2, main = '')
...
hist(posterior.extend.draws[["Sigma.posterior.exten"]][1,6,][(S1+1):total_S], xlab = "M3 sigma", col = green2, main = '')
```
