...
#| message: false
#| warning: false
#| fig-cap: "Table 2 Basic Model proofing simulation for A"
...
#| message: false
#| warning: false
#| fig-cap: "Table 3 Basic Model proofing simulation for Sigma"
...
#| message: false
#| warning: false
#| fig-cap: "Table 4 Extend Model proofing simulation for A"
...
#| message: false
#| warning: false
#| fig-cap: "Table 5 Extend Model proofing simulation for Sigma"
...
point.lcpi.f.ext    = apply(Y.h.ext[,1,],1,mean)
interval.lcpi.f.ext = apply(Y.h.ext[,1,],1,hdi,credMass=0.90)
lcpi.range.ext      = range(y[,1],interval.lcpi.f.ext)

point.infexp.f.ext       = apply(Y.h.ext[,2,],1,mean)
interval.infexp.f.ext    = apply(Y.h.ext[,2,],1,hdi,credMass=0.90)
infexp.range.ext        = range(y[,2],interval.infexp.f.ext)

par(mfcol = c(2,1), mar=c(2,2,2,2)-0.1)

plot(1:(length(y[,1])+h),c(y[,1],point.lcpi.f.ext), type="l", ylim=c(3.9, 5.1), axes=FALSE, xlab="lcpi", ylab="", lwd=2, col=blue1)
axis(1,c(1,41,81,121,nrow(y),nrow(y)+h),c("1990","2000","2010","2020","",""), col=green1)
axis(2, at = pretty(lcpi.range.ext), labels = pretty(lcpi.range.ext), col = green1)
abline(v=137, col=green1)
polygon(c(length(y[,1]):(length(y[,1])+h),(length(y[,1]):(length(y[,1])+h))[9:1]),
        c(y[137,1],interval.lcpi.f.ext[1,],interval.lcpi.f.ext[2,8:1],y[137,1]),
        col=blue4.shade1, border=blue4.shade1)

plot(1:(length(y[,2])+h),c(y[,2],point.infexp.f.ext), type="l", ylim=c(0,5), axes=FALSE, xlab="infexp", ylab="", lwd=2, col=green6)
axis(1,c(1,41,81,121,nrow(y),nrow(y)+h),c("1990","2000","2010","2020","",""), col=green1)
axis(2, at = pretty(infexp.range.ext), labels = pretty(infexp.range.ext), col = green1)

abline(v=137, col=green1)
polygon(c(length(y[,2]):(length(y[,2])+h),(length(y[,2]):(length(y[,2])+h))[9:1]),
        c(y[137,2],interval.infexp.f.ext[1,],interval.infexp.f.ext[2,8:1],y[137,2]),
        col=green3.shade1, border=green3.shade1)
...
lcpi.data.h.ext             = c(lcpi_data["/2024-03-31"], point.lcpi.h.ext)
...
infexp.data.h.ext           = c(infexp_data["/2024-03-31"], point.infexp.h.ext  )

key_data.h.ext              = na.omit(merge(inf.data.h.ext, infexp.data.h.ext))
colnames(key_data.h.ext)    = c("inf_data.h", "infexp_data.h")
...
legend("topleft", legend = c("Inflation Rate", "Expected Inflation Rate", "Inflation Rate Forecast", "Expected Inflation Rate Forecast", "2024-03-31"),
       col = c(blue1, green2, blue4, green3, green1), lty = 1, cex = 0.6)
...
Until now, we acknowledge that our model has not achieved the most optimal results. Our modeling approach has operated under the assumption of homoscedasticity. However, real-world events, such as the 2008 financial crisis and the 2020 COVID-19 pandemic, have demonstrated the significant and often abrupt fluctuations in the economy and financial markets. These events underscore the necessity of incorporating heteroscedasticity into our model to more accurately reflect the variable nature of economic conditions and market volatility.
...
posterior.draws.hetero = function (total_S, Y, X){
...
## Function Applying
posterior.hetero = posterior.draws.hetero(total_S = total_S, Y = Y, X = X) 
...
# Ensure the columns are vectors
data_df$inf_data.h <- as.vector(data_df$inf_data.h)
data_df$infexp_data.h <- as.vector(data_df$infexp_data.h)
...
legend("topright", legend = c("Inflation Rate", "Expected Inflation Rate", "Inflation Rate Forecast", "Expected Inflation Rate Forecast", "2024-03-31"),
       col = c(blue1, green2, blue4, green3, green1), lty = 1, cex = 0.6)
# title(main = "Inflation Rate vs Expected Inflation Rate Forecasing by Basic Model with heteroskedasticity")
