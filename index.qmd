...
ggplot(data_df, aes(x = Index)) +
  geom_rect(aes(xmin = min(Index), xmax = max(Index), ymin = 2, ymax = 3), fill = blue2, alpha = 0.5) +
  geom_line(aes(y = inf_data, colour = "Inflation Rate")) +
  geom_line(aes(y = infexp_data, colour = "Expected Inflation Rate")) +
  scale_color_manual(values = c("Inflation Rate" = blue1, "Expected Inflation Rate" = blue4)) +
...
Since probability density function of Gamma distribution is:
...
We can figure out the kernel is: $x^{k-1}e^{-\frac{x}{\theta}}$
...
&\propto 
 \lambda^{-\frac{N}{2} + \underline{k_{\lambda}}-1 } exp\left\{ -\frac{1}{2\lambda} \mathrm{tr}\left[ \Sigma^{-1} (A-\underline{A})' (\underline{V})^{-1} (A-\underline{A})\right]-\frac{\lambda}{\underline{\theta_{\lambda}}} \right\} 
\end{align}
$$
