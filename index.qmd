...
After first-order difference processing of the data and logarithmic transformation of part of the data, the line plot of the dataset is as shown below (Figure 3): Although the fluctuation amplitude cannot be directly compared due to the different proportions on the y-axis, from Looking at the trend of fluctuations, $gold_i$ are negatively correlated with $lcpi_i$, and $gdp_i$ and $crt_i$ are positively correlated. This finding is consistent with the data presented in the matrix of correlations.
...
Observing the trend charts of gold prices and cpi in the past three years, we can see that the trends of the two are almost consistent, so we believe that it is reasonable to use international gold prices as inflation expectations (gold price reduced by tenfold). Although the gold price is denominated in US dollars, the inclusion of the Australian dollar against the US dollar exchange rate in the data set makes it feasible to convert the gold price into a measure of its impact on the Australian market.
