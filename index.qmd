...
In summary, the dataset included data from 1990Q2 to 2023Q4, total 135 observation points with 10 variables.
...
dadf.export = adfTest(diff(all_data[,7]), lags=5, type="nc")    # don't reject -> non-stationary
...
ggplot(data_df, aes(x = Index)) +
  ...
  scale_fill_manual(values = c("Target Inflation Zone" = color2), 
                    name = "Legend", 
                    guide = guide_legend(override.aes = list(alpha = 0.5))) +
  scale_color_manual(values = c("Inflation Rate" = color1, 
                                "Expected Inflation Rate" = color4)) +
...
$$
X=\begin{bmatrix}
1 & y_{lcpi,t-1} & y_{infexp,t-1} & \ldots & y_{exr,t-1} & \ldots & y_{lcpi,t-p} & y_{lgold,t-p} & \ldots &  y_{exr,t-p} \\
1 & y_{lcpi,t-2} & y_{infexp,t-2} & \ldots & y_{exr,t-2} & \ldots & y_{lcpi,t-p-1} & y_{lgold,t-p-1} & \ldots &  y_{exr,t-p-1} \\
\vdots & \vdots & \vdots & \ddots & \vdots & \ddots & \vdots & \vdots &  \ddots & \vdots \\
1 & y_{lcpi,1} & y_{infexp,1} & \ldots & y_{exr,1} & \ldots & y_{lcpi,1-p} & y_{lgold,1-p} & \ldots &  y_{exr,1-p}\\
...
The basic model based on the natural-conjugate prior distribution, which is specified as a matrix normal inverse Wishart distribution. Minnesota prior with some stylised facts about the macroeconomic time series, since $cpi_t$ and $m_t$ are unit root non-stationary and other all variables are stationary, is applied to form the specifications of the prior distribution.
...
-   $\underline{A}$ is a $K \times N$ matrix, reflect the random walk with no drift process on the first lag of $cpi_{t}$, and 0 elsewhere.
...
$$
\underline{V} = diag\left[ \underbrace{\kappa_{2}}_{intercept} \quad \underbrace{\kappa_{1}(\textbf{p} ^{-2}\otimes l^{'}_{N})}_{A_{1} \ to \ A_{p}}  \right]
$$

-   $\underline{S}$ is $10\times10$ symmetric matrix where the diagonal represents the variances of individual variables and the off-diagonals represent the covariances between the variables.

-   $\underline{v}$ is degree of freedom $N+1$, and it is a single values because variance is assumed to be the same for all elements of $\Sigma$.\
