...
-   **Economic Activity and Policy Indicators:**
    ...
    4.  $crt_i$: Cash rate target publsh by RBA.
...
In summary, the dataset included data from 2010Q1 to 2023Q4, total 96 observation points with 10 variables.


### Data Transformation

-   **Quarterly Transformation**
All data have been quarterly converted by selecting the data on the last day of the quarter as the observation.
...
According to Table 2, we can see that $lcpi_i$, $gold_i$, $lexport_i$ require  second-order integration, while $gdp_i$, $unemp_i$, $nloan_i$ require first-order integration(under 5% level of confidence and n in the table means larger than 2).
...
$$y_t = \mu_0 + A_1y_{t-1} +...+A_1y_{t-p} +\epsilon_t$$

$$\epsilon_t|Y_{t-1} \sim iid \mathcal{N}_N(0_N, \Sigma)$$
...
1 & y_{lcpi,1} & y_{gold,2} & \ldots & y_{lexr,1} & \ldots & y_{lcpi,1-p} & y_{gold,1-p} & \ldots &  y_{lexr,1-p}\\
...
In the forecasting process, we will concentrate on determining the conditional mean and confidence interval of the projected CPI, denoted as $cpi_{T+h|T}$. Subsequently, we'll utilize the forecasted CPI values to calculate the inflation rate and assess whether it aligns with the target inflation range of 2-3%.
