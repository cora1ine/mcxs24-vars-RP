...
posterior.draws.exten = function (total_S, Y, X){
  ...
  # A.posterior
  # lambda.priors
  # all the object that are used in this function must be defined inside 
  # of it or provided as arguments of the function.
  # it is not a good programming practice to rely on global variables.
...
#axis(2,c(lcpi.range[1],mean(lcpi.range),lcpi.range[2]),c("","lcpi",""), col=green1)
axis(2, at = pretty(lcpi.range), labels = pretty(lcpi.range), col = green1)
abline(v=136, col=green1)
polygon(c(length(y[,1]):(length(y[,1])+h),(length(y[,1]):(length(y[,1])+h))[9:1]),
        c(y[136,1],interval.lcpi.f[1,],interval.lcpi.f[2,8:1],y[136,1]),
...
axis(1,c(1,41,81,121,nrow(y),nrow(y)+h),c("1990","2000","2010","2020","",""), col=green1)
#axis(2,c(infexp.range[1],mean(infexp.range),infexp.range[2]),c("","infexp",""), col=green1)
...
polygon(c(length(y[,2]):(length(y[,2])+h),(length(y[,2]):(length(y[,2])+h))[9:1]),
        c(y[136,2],interval.infexp.f[1,],interval.infexp.f[2,8:1],y[136,2]),
...
Figure 7 2D forecasting graph on basic model
...
Figure 8 Basic Model Key Data Plot
...
Figure 9 3d forecasting graph on extension model
...
abline(v=136, col=green1)
polygon(c(length(y[,1]):(length(y[,1])+h),(length(y[,1]):(length(y[,1])+h))[9:1]),
        c(y[136,1],interval.lcpi.f.ext[1,],interval.lcpi.f.ext[2,8:1],y[136,1]),
        col=blue4.shade1, border=blue4.shade1)
...
abline(v=136, col=green1)
polygon(c(length(y[,2]):(length(y[,2])+h),(length(y[,2]):(length(y[,2])+h))[9:1]),
        c(y[136,2],interval.infexp.f.ext[1,],interval.infexp.f.ext[2,8:1],y[136,2]),
        col=green3.shade1, border=green3.shade1)
...
Figure 10 2d forecasting graph on extension model
...
Figure 11 Extend Model Key Data Plot
...
for (i in 1:4) {
  ...
  print(interval.lcpi.f.ext[1,])
  print(interval.lcpi.f.ext[2,h:1])
  for (i in 1:h) {
    f4.l <- trans3d(x = x, y = yy[i], z = z[,i], pmat = f4)
...
Figure 12 predictive density of lcpi with different k and theta

The predictive density distribution remains stable despite changes in the prior distribution of the Gamma distribution in lambda, indicating that the predictive values have stabilized in the Gibbs sampler. This suggests that with sufficient data, the influence of the prior distribution is overshadowed by the data, resulting in a stable posterior distribution and predictive outcome. Consequently, the predictive results are primarily driven by the data rather than the prior assumptions.
