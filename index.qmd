...
ggplot(data_df, aes(x = Index)) +
  geom_ribbon(aes(ymin = 2, ymax = 3, fill = "Target Inflation Zone"), 
  ...
  scale_fill_manual(values = c("Target Inflation Zone" = blue2), 
                    name = "Legend", 
                    guide = guide_legend(override.aes = list(alpha = 0.5))) +
  scale_color_manual(values = c("Inflation Rate" = blue1, 
                                "Expected Inflation Rate" = blue4)) +
  labs(title = "Inflation Rate vs Expected Inflation Rate", 
       x = "Date", y = "Rate (%)") +
  theme_minimal() +
...
The basic model based on the natural-conjugate prior distribution, which is specified as a matrix normal inverse Wishart distribution [@wozniakBsvarsBayesianEstimation2022]. Minnesota prior with some stylised facts about the macroeconomic time series, since $cpi_t$ and $m_t$ are unit root non-stationary and other all variables are stationary, is applied to form the specifications of the prior distribution.
...
$$A|\Sigma \sim \mathcal{MN}_{K\times N}(\underline{A}, \Sigma,\underline{V} ) $$ $$\Sigma \sim \mathcal{IW}_{N}(\underline{S}, \underline{v})$$

-   $\underline{A}$ is a $K \times 10$ matrix, reflect the random walk with no drift process on the first lag of $cpi_{t}$, and 0 elsewhere.

$$
\underline{A} = \left[ 
\underbrace{\textbf{0}_{10\times 1}}_{intercept} \quad 
\underbrace{\textbf{I}_{10} \quad\begin{matrix}
0 & \cdots  &  0\\
\vdots  & \ddots  & \vdots  \\
0 & \cdots  & 0
\end{matrix}_{10\times10(p-1))}}_{N\times(K-1)} \quad 
\right]'
$$
...
-   insert it in $\mathcal{MN}_{K\times 10}(\overline{A}, \Sigma,\overline{V} )$ to draw $A$
...
```{r Basic Model 1, fig.align='center',fig.pos='H'}
#| echo: true
#| message: false
#| warning: false
...
Below are the trace plots and histograms posterior to draws on $A$ and $\Sigma$. It is clear from the trace plots that the inflation has a very strong perfect positive correlation with its own preceding period, which is consistent with the previous distribution design and the anticipation of integration at 1. 


```{r, fig.align='center',fig.pos='H'}
par(mfcol = c(2, 2), mar = c(4, 2, 2, 2))
plot.ts(posterior.sample.draws[["A.posterior"]][2,1,], xlab = "Simulation times S", ylab = "CPI", col = blue6)    # 2 row 1 col of matrix A 
hist(posterior.sample.draws[["A.posterior"]][2,1,], xlab = "CPI", col = blue2, main = '')
plot.ts(posterior.sample.draws[["Sigma.posterior"]][1,1,], xlab = "Simulation times S", ylab = "CPI sigma", col = green4) 
