...
```{r Plot inf and infexp, fig.align='center',fig.pos='H'}
#| echo: false
#| message: false
#| warning: false
...
plot(data_df$Index, data_df$inf_data, type = "l", col = blue1,
     xlab = "Date", ylab = "Rate (%)", ylim = c(0, max(c(data_df$inf_data, data_df$infexp_data))))
...
lines(data_df$Index, data_df$infexp_data, col = blue4)

legend("topright", legend = c("Inflation Rate", "Expected Inflation Rate"),
       col = c(blue3, blue4), lty = 1, cex = 0.8)
...
\\
 p(\Sigma|Y,X) &\sim \mathcal{IW}_{N}(\overline{S}, \overline{v})
 \end{align}
$$
...
## Function Applying
posterior.sample.draws = posterior.draws(S=5000, Y=Y, X=X) # sample use S=5000

```

Below are the histograms posterior to draws on $A$ and $\Sigma$. It is clear that CPI has a very strong perfect positive correlation with its own preceding period, which is consistent with the previous distribution design and the anticipation of integration at 1.

```{r Basic Model Plot, fig.align='center',fig.pos='H'}
#| echo: false
#| message: false
#| warning: false

par(mfcol = c(2, 2), mar = c(4, 2, 2, 2))
# 2 row 1 col of matrix A 

hist(posterior.sample.draws[["A.posterior"]][2,1,], xlab = "CPI", col = blue2, main = '')
hist(posterior.sample.draws[["A.posterior"]][2,2,], xlab = "Inflation Expectation", col = blue2, main = '')

hist(posterior.sample.draws[["Sigma.posterior"]][1,1,], xlab = "CPI sigma", col = green2, main = '')
hist(posterior.sample.draws[["Sigma.posterior"]][1,2,], xlab = "Inflation Expectation sigma", col = green2, main = '')
...
+
\left[ \begin{matrix}
\epsilon^{(1)}_{t+1} \\ \epsilon^{(2)}_{t+1}
\end{matrix} \right] \\
\epsilon^{(1)}_{t+1} \sim \mathcal{N}(0,1) \\
\epsilon^{(2)}_{t+1} \sim \mathcal{N}(0,1)
$$
...
# Applying function 
posterior.sample.draws.p = posterior.draws(S=1, Y=Y, X=X)

```



```{r Basic Model Proof, fig.align='center',fig.pos='H'}
#| echo: false
#| message: false
#| warning: false

basic.model.proof <- 
  tibble( " " = c("Constant term", "1 lag"),
          "Simulation Parameter" 
          = c(round(mean(posterior.sample.draws.p[["A.posterior"]][1,1,]),6),
              round(mean(posterior.sample.draws.p[["A.posterior"]][2,1,]),6))

  )


kable(basic.model.proof, align = "c") %>% 
  kable_styling(font_size = 8, 
                fixed_thead = TRUE, 
                full_width = FALSE, 
                position = "center",
...
Table 2 Basic Model Proofing Simulation
...
p(\Sigma) &= det(\Sigma)^{\frac{\underline{v}+N+1}{2}}exp\left\{ -\frac{1}{2} \mathrm{tr}\left[ \Sigma^{-1} \underline{S}\right]\right\}  \\
...
S2                = 4500                          # number of draws from the final simulation
total_S           = S1+S2
...
# Presenting output
# head(round(apply(posterior.extend.draw$A.posterior.exten[,,(S1+1):total_S], 1:2, mean),6))
```


By including the $\lambda$ in the variance specification of the prior mean A, it adds more variability than the basic model. As opposed to having a constant shrinkage level, variance of A for Extend model is non-constant. This modification produces noticeable effects on the CPI, whose coefficient now spans a wider range from 0.9 to 1.05 as opposed to the basic model's 0.98 to 1.01. 


```{r Extension Model Plot, fig.align='center',fig.pos='H'}
#| echo: false
#| message: false
#| warning: false

par(mfcol = c(3, 2), mar = c(4, 2, 2, 2))
# 2 row 1 col of matrix A 

hist(posterior.extend.draws[["A.posterior.exten"]][2,1,][(S1+1):total_S], xlab = "CPI", col = blue2, main = '')
hist(posterior.extend.draws[["A.posterior.exten"]][2,2,][(S1+1):total_S], xlab = "Inflation Expectation", col = blue2, main = '')

hist(posterior.extend.draws[["Sigma.posterior.exten"]][1,1,][(S1+1):total_S], xlab = "CPI sigma", col = green2, main = '')
hist(posterior.extend.draws[["Sigma.posterior.exten"]][1,2,][(S1+1):total_S], xlab = "Inflation Expectation sigma", col = green2, main = '')

```

Figure 7 Model Simulation of Extend Model

```{r Extension Model lambda Plot, fig.align='center',fig.pos='H'}
#| echo: false
#| message: false
#| warning: false

par(mfcol = c(1, 2), mar = c(14, 2, 4, 2))
plot.ts(posterior.extend.draws[["lambda.posterior.exten"]][(S1+1):total_S], xlab = "Simulation times S", ylab = "lambda", col = blue1)   
hist(posterior.extend.draws[["lambda.posterior.exten"]][(S1+1):total_S], xlab = "lambda", col = green1, main = '')
```

Figure 6 Model Simulation of Extend Model (lambda)
...
S2          = 900                             # number of draws from the final simulation
total_S     = S1+S2
A.posterior       = array(NA, dim = c((1+N*p),N,S1+S2))
Sigma.posterior   = array(NA, dim = c(N,N,S1+S2))
...
```{r Extend Model Proof, fig.align='center',fig.pos='H'}
#| echo: false
#| message: false
#| warning: false
extend.model.proof <-
  tibble( " " = c("Constant term", "1 lag"),
          "Simulation Parameter" 
          = c(round(mean(posterior.extend.draws.p[["A.posterior.exten"]][1,1,]),6),
              round(mean(posterior.extend.draws.p[["A.posterior.exten"]][2,1,]),6))

  )

kable(extend.model.proof, align = "c") %>% 
  kable_styling(font_size = 8, 
                fixed_thead = TRUE, 
                full_width = FALSE, 
                position = "center",
...
Table 3 Extend Model Proofing Simulation
